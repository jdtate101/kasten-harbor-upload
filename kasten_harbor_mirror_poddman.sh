...
#
# Requires: podman, credentials file (see CRED_FILE below)
#
...
banner() {
    ...
    echo -e "${C_BOLD}${C_CYAN}   Kasten K10 -> Harbor image mirror${C_RESET}"
    echo -e "${C_BOLD}${C_CYAN}=====================================================${C_RESET}"
}
...
command -v podman >/dev/null 2>&1 || { err "podman not found in PATH"; exit 1; }
...
if podman login "$REGISTRY" --username "$HARBOR_USER" --password "$HARBOR_PASS" --tls-verify=false >>"$LOG_FILE" 2>&1; then
...
if ! podman pull "${UPSTREAM_HOST}/kasten-images/k10tools:${VERSION}" >>"$LOG_FILE" 2>&1; then
...
IMAGE_LIST_RAW=$(podman run --rm "${UPSTREAM_HOST}/kasten-images/k10tools:${VERSION}" image list 2>>"$LOG_FILE" | tr -d '\r')
...
for img in "${SOURCE_IMAGES[@]}"; do
    ...
    if run_with_spinner "$i" "$TOTAL" "pull  ${short}" podman pull "$img"; then
        PULLED+=("$img")
    else
        FAILED_PULL+=("$img")
    fi
...
confirm "Tag and push these ${#PULLED[@]} image(s) to ${REGISTRY}?" || { warn "Aborted by user before push - images remain in local podman storage."; exit 0; }
...
for src in "${PULLED[@]}"; do
    ...
    if ! podman tag "$src" "$dest" >>"$LOG_FILE" 2>&1; then
        err "Failed to tag $src as $dest"
        FAILED_PUSH+=("$dest")
        continue
    fi

    if run_with_spinner "$i" "$TOTAL_PUSH" "push  ${short}" podman push --tls-verify=false "$dest"; then
        PUSHED+=("$dest")
    else
        FAILED_PUSH+=("$dest")
    fi
